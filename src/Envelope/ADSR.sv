...
module adsr
    #(
        ...
        parameter int ENV_WIDTH   = OPT_DEPTH,
        parameter int ENV_FRACT   = (OPT_DEPTH / 2)
    )
    (
        input  logic clk,
        input  logic rst_n,
        ...
        input adsr_params_t params,

//        // -- ADSR Coefficients (Unsigned, representing step size per sample) --
//        input  logic [ENV_WIDTH-1:0] params.attack_time,
//        input  logic [ENV_WIDTH-1:0] params.decay_time,
//        input  logic [ENV_WIDTH-1:0] params.release_time,
//        // Sustain level is an amplitude target, not a step rate (Q16 format, max 65536)
//        input  logic [ENV_WIDTH-1:0] params.sustain_ampl,
            ...
            if (state != IDLE && state != RELEASE && !gate) begin
                ...
                case (state)
                    ...
                    ATTACK: begin
                        ...
                        if ((MAX_ENV - env_val) <= params.attack_time) begin
                            env_val <= MAX_ENV;
                            state   <= DECAY;
                        end else begin
                            env_val <= env_val + params.attack_time;
                        end
                    end

                    DECAY: begin
                        // Prevent underflow to sustain level
                        if (env_val <= (params.sustain_ampl + params.decay_time)) begin
                            env_val <= params.sustain_ampl;
                            state   <= SUSTAIN;
                        end else begin
                            env_val <= env_val - params.decay_time;
                        end
                    end
                    ...
                    RELEASE: begin
                        ...
                        if (env_val <= params.release_time) begin
                            env_val <= '0;
                            state   <= IDLE;
                        end else begin
                            env_val <= env_val - params.release_time;
                        end
                    end
