...
module ADSR_demo_day
    #(
        parameter BIT_DEPTH = 24,
        parameter  OPTION_DEPTH = 8
      )
     (
        input logic clk, rst_l, update_en,
        input logic [OPTION_DEPTH-1:0] attack_time,
        input logic [OPTION_DEPTH-1:0] decay_time,
        input logic [OPTION_DEPTH-1:0] sustain_level,
        input logic [OPTION_DEPTH-1:0] release_time,
        ...
        always_ff @(posedge clk or negedge rst_l) begin
            if (!rst_l) begin
                ...
                release_done <= 0;
                 ...
                 if (retrig_window & gate) begin //reset on retrig and 
                    retrig_window <= 0;
                    envelope_level <= 0;
                    release_done <= 0;
                    decay_done <= 0;
                ...
                //key press: A -> D -> latch at sustained level
                    //ATTACK
                    if (!attack_done) begin
                        //basic increment
                        envelope_level <= envelope_level + attack_incr;
                        ...
                        if (envelope_level > ('1 - attack_incr)) begin
                            attack_done <= 1;
                            envelope_level <= '1;
                        end
                    end
                    ...
                    //DECAY
                        ...
                        envelope_level <= envelope_level - decay_incr;
                        //guard to move-to sustain
                        if ({sustain_level, 16'b0} + decay_incr 
                                >= envelope_level) begin
                            envelope_level <= {sustain_level, 16'b0};
                            decay_done <= 1;
                        end
                    end
                ...
                //on key-off: start release
                    ...
                    if (envelope_level <= release_incr) begin
                        envelope_level <= 0;
                        release_done <= 1;
                    end
                end
